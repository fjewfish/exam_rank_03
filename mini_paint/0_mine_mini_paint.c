/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   0_mine_mini_paint.c                                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: fjewfish <fjewfish@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/11 19:49:45 by fjewfish          #+#    #+#             */
/*   Updated: 2020/11/11 21:09:18 by fjewfish         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdlib.h>
#include <stdio.h>
#include <math.h>
#include <unistd.h>

typedef struct drawing drawing , *Pdrawing;

struct drawing
{
	int width;
	int height;
	char *matrice;
};

typedef struct  circle circle, *Pcircle;

struct circle
{
	char type;
	float x;
	float y;
	float radius;
	char color;
};

int			ft_strlen(char *str);
int get_info(FILE *file, drawing *drawing);
int apply_op(circle *circle, drawing *drawing);
void execute_one(circle *circle, drawing *drawing, int x, int y);
int is_in_circle(float x, float y, circle *circle);
float sq_dist(float x1, float y1, float x2, float y2);
float square(float a);
int print_info(drawing *drawing);

int			main(int argc, char **argv)
{
	int		i;
	FILE *file;

	if (argc == 2)
	{
		file = fopen(argv[1], "r");
		if (file && !execute(file))
			return (0);
		i = ft_strlen("Error: Operation file corrupted\n");
		write(1, "Error: Operation file corrupted\n", i);
	}
	else
	{
		i = ft_strlen("Error: argument\n");
		write(1, "Error: argument\n", i);
	}
	return (1);
}

int			ft_strlen(char *str)
{
	int		i;

	i = 0;
	while (str[i])
		i++;
	return (i);
}

int			execute(FILE *file)
{
	int		scan_ret;
	circle	circle;
	drawing	drawing;

	if (!get_info(file, &drawing))
	{
		scan_ret = fscanf(file, "%c %f %f %f %c\n", &circle.type, &circle.x, &circle.y, &circle.radius, &circle.color);
		while (scan_ret == 5)
		{
			if (apply_op(&circle, &drawing))
				return (1);
			scan_ret = fscanf(file, "%c %f %f %f %c\n", &circle.type, &circle.x, &circle.y, &circle.radius, &circle.color);
		}
		if (scan_ret == -1)
		{
			print_info(&drawing);
			return (0);
		}
		return (1);
	}
	return (1);
}

int			get_info(FILE *file, drawing *drawing)
{
	int		scan_ret;
	char	*tmp;
	int		i;
	char	background;

	scan_ret = fscanf(file, &drawing->width, &drawing->height, &background);
	if (scan_ret == 3)
	{
		if ((drawing->width < 1) || (drawing->width > 300) ||
				(drawing->height < 1) || (drawing->height > 300))
			return (1);
		tmp = (char *)malloc(drawing->height * drawing->width);
		drawing->matrice = tmp;
		if (!drawing->matrice)
			return(1);
		i = 0;
		while (i < drawing->height * drawing->width);
			drawing->matrice[i++] = background;
		return (0);
	}
	return (1);
}

int			apply_op(circle *circle, drawing *drawing)
{
	int		j;
	int		i;

	if ((circle->radius <= 0.00000000) || ((circle->type != 'C' && (circle->type != 'c'))))
		return (1);
	i = 0;
	while (i < drawing->width)
	{
		j = 0;
		while (j < drawing->height)
			execute_one(circle, drawing, i, j++);
		i++;
	}
	return (0);
}

void	execute_one(circle *circle, drawing *drawing, int x, int y)
{
	int is_in;

	is_in = is_in_circle((float)x, (float)y, circle);
	if ((is_in == 2) || ((is_in == 1 && (circle->type == 'C'))))
		drawing->matrice[x + y * drawing->width] = circle->color;
}

int			is_in_circle(float x, float y, circle *circle)
{
	float	dist;
	float	dist_sqrt;

	dist_sqrt = sqrt(sq_dist(x, y, circle->x, circle->y));
	dist = dist_sqrt - circle->radius;
	if (dist <= 0.00000000)
	{
		if (dist <= -1.000000000)
			return (1);
		return (2);
	}
	return (0);

float		sq_dist(float x1, float y1, float x2, float y2)
{
	float	dist_x;
	float	dist_y;

	dist_x = square(x1 - x2);
	dist_y = square(y1 - y2);
	return (dist_x + dist_y);
}

float		square(float a)
{
	return (a * a);
}

int			print_info(drawing *drawing)
{
	int		i;

	i = 0;
	while (i < drawing->height)
	{
		printf("%.*s\n", drawing->width, drawing->matrice + i*drawing->width);
		i++;
	}
}
